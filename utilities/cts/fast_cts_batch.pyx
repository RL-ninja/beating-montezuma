...
cdef void cts_update(CTSStruct* cts, int[:] context, int symbol):
    ...
    node_update(cts[0]._root, context, symbol)
...
cdef class CTS:
    ...
    cpdef void update(self, int[:] context, int symbol):
        cts_update(self.inner, context, symbol)
...
cdef class CTSDensityModel:
    ...
    cpdef (double, double) _get_probs(self, int[:, :] obs):
        cdef int[:] context = np.array([0, 0, 0, 0], np.int32)
        cdef double log_prob = 0.0
        cdef double log_recoding_prob = 0.0
        cdef unsigned int i
    ...
    def get_exploration_bonus(self, obs):
        obs = self._quantize_obs(obs)
        # simply query the model
        log_prob, log_recoding_prob = self._get_probs(obs)
